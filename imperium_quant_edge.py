...
st.markdown("**Kill Switch Working**")
...
if auto_trading and st.session_state.get('connected') and (continuous or burst):
    ...
    for _ in range(8):  # Controlled cycles
        if not st.session_state.trading_active:
            break
            ...
            for sym in markets:
                if random.random() < 0.45:
                    try:
                        ...
                        pass
        time.sleep(0.8 if burst else 1.5)

st.subheader("Status")
st.info("Turn on Auto Trading + Continuous/Burst, then use the Kill Switch to stop.")
