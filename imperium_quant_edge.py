# ============================================================
#  IMPERIUM QUANT EDGE v7.2 — Adaptive RL Trading Platform
...
#  - IBKR-synced balance & withdrawable funds (v7.2)
...
class RLConfig:
    ...
    base_lr: float = 0.25             # base learning rate
    min_lr: float = 0.05
    max_lr: float = 0.60
    meta_strength: float = 0.15       # how fast LR adapts
    pattern_weight: float = 0.35
    memory_weight: float = 0.25
    reward_vol_boost: float = 1.4
    reward_trend_boost: float = 1.2
    max_memory: int = 20              # LSTM-style memory window
...
class AccountState:
    ...
    withdrawable_balance: float = 100_000.0   # anchored to IBKR AvailableFunds when possible


@dataclass
class EngineConfig:
    mode: str = "Hybrid"              # Simulated / Live / Hybrid
    ai_aggressiveness: float = 1.0    # v7.2 more aggressive by default
    ...
    # Meta-policy mode (label only)
    meta_mode: str = "ADAPTIVE"

    # v7.1 leverage controls
    max_leverage: float = 10.0
    min_leverage: float = 1.0
    current_leverage: float = 3.0


# v7 margin requirements
MARGIN_REQUIREMENTS = {
    "EURUSD": 0.02,
    "GBPUSD": 0.02,
    "USDJPY": 0.02,
...
# ============================================================
#  IBKR CONTRACT BUILDER (FX + XAU + BTC)
# ============================================================
...
# ============================================================
#  IBKR CONNECTION + ACCOUNT SYNC (v7.2)
# ============================================================
...
def sync_account_from_ibkr():
    """Sync balance and withdrawable funds from IBKR account summary."""
...
# ============================================================
#  STATE ENGINE + FEATURE EXTRACTION
# ============================================================
...
# ============================================================
#  RL CORE (Reward, Policy Update, TD Learning)
# ============================================================
...
def shaped_reward(m: MarketState, raw_pnl: float, eng: EngineConfig) -> float:
    ...
    # v7.1: portfolio Sharpe-like adjustment
    acct = st.session_state.account
...
def score_to_action(score: float, threshold: float = 0.05) -> Optional[str]:
    """v7.2: lower threshold so trades actually fire."""
    if score > threshold:
        return "BUY"
...
def amplify_score_by_mode(score: float, mode: str) -> float:
    if mode == "CONSERVATIVE":
        return score * 1.0   # v7.2: no dampening in low-vol
    if mode == "AGGRESSIVE":
        return score * 1.2
    if mode == "PREDATOR":
        return score * 1.6
...
# ============================================================
#  EXECUTION ENGINE + IBKR INTEGRATION
# ============================================================
...
def apply_slippage(price: float, mode: str) -> float:
    if mode == "CONSERVATIVE":
        return price * 1.0001
    if mode == "AGGRESSIVE":
        return price * 1.0003
...
# ============================================================
#  v7 — MARGIN + PORTFOLIO RL + LEVERAGE + NETTING
# ============================================================
...
def update_margin(acct: AccountState, mkts: dict):
    ...
    # Use IBKR free_margin as upper bound if available
    model_free = equity - acct.margin_used
    if acct.free_margin is None:
        acct.free_margin = max(0.0, model_free)
    else:
        acct.free_margin = min(acct.free_margin, max(0.0, model_free))
...
def run_trading_cycle():
    ...
    # v7.2: keep account synced with IBKR each cycle
    sync_account_from_ibkr()
    ...
    for symbol, m in mkts.items():
        ...
        fast, slow = compute_factor_scores(
            symbol, price, last_price, eng.ai_aggressiveness
        )
        m.score_fast = fast
        m.score_slow = slow
        ...
        state_vec = build_state_vector(m)
...
def render_ui():
    st.title("⚔️ Imperium Quant Edge v7.2 — Adaptive RL Trading Platform")
    ...
    st.metric("Withdrawable Balance (IBKR cap)", f"${acct.withdrawable_balance:,.2f}")
    st.metric("Margin Used (model)", f"${acct.margin_used:,.2f}")
    st.metric("Free Margin (min(IBKR, model))", f"${acct.free_margin:,.2f}")
    ...
    st.write(f"Current Leverage: {eng.current_leverage:.2f}x")
