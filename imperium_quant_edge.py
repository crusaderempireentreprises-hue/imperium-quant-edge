...
st.markdown("**Smart Adaptive AI - Full Stable Version**")
...
client_id = st.sidebar.number_input("Client ID", value=53)
...
def get_contract(sym):
    try:
        if sym == "XAUUSD":
            ...
            clean = sym.replace(".", "")   # USDCAD → USDCAD
            return Forex(clean)
    except:
        return None
...
if st.button("▶️ SEND TRADING CYCLE NOW", type="primary"):
    if st.session_state.get('connected'):
        ...
        st.info("Sending new smart trades...")
        success_count = 0
        ...
        st.info(f"Cycle finished — {success_count} trades sent")
    else:
...
with st.container():
    ...
    3. Click "SEND TRADING CYCLE NOW" multiple times  
    4. Check TWS → Trades tab
    """)
