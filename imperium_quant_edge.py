...
# Trading Button (No infinite loop)
if st.button("▶️ Start Trading Now"):
    if auto_trading and st.session_state.get('connected'):
        ib = st.session_state.ib
        st.success("Trading Started - Check TWS for orders")
        for _ in range(8):   # Limited cycles to prevent freezing
            for sym in markets:
                if random.random() < 0.4:
                    try:
                        ...
                        size = max(1, int(1000 * 0.01 / 100))  # small size for safety
                        ib.placeOrder(contract, MarketOrder(signal, size))
                        st.write(f"✅ Order: {signal} {size} {sym}")
                    except:
                        pass
            time.sleep(1)
...
with st.container():
    ...
    st.info("Click **Start Trading Now** to test. Check TWS Trades tab.")
