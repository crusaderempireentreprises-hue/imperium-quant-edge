...
continuous_trading = st.sidebar.toggle("🔄 Continuous Trading (Max 20 trades)", value=False)
burst_mode = st.sidebar.toggle("⚡ Burst Mode (No trade limit)", value=False)
...
if continuous_trading or burst_mode:
    current_speed = 0.4 if burst_mode else 1.5
    mode_name = "Burst Mode ⚡" if burst_mode else f"Continuous Mode (Max 20 trades)"
    st.success(f"🚀 {mode_name} ACTIVE")
    
    placeholder = st.empty()
    
    while (continuous_trading or burst_mode) and st.session_state.trade_count < 20:
        with placeholder.container():
            ...
            st.metric("Trades Executed", f"{st.session_state.trade_count} / 20")
        
        time.sleep(current_speed)

        if continuous_trading and st.session_state.trade_count >= 20:
            st.warning("Maximum 20 trades reached in Continuous mode.")
...
st.caption("Imperium Quant Edge • Clean Version")