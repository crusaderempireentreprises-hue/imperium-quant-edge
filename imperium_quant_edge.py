        ...
        try:
            ...
            for item in summary:
                if item.tag == "NetLiquidation":
                    ...
                    st.success(f"✅ Updated: ${balance:,.2f}")
                    break
        except Exception as e:
...
markets = st.sidebar.multiselect("Select Markets", 
    ["XAUUSD", "EUR.USD", "USD.CAD", "GBP.USD", "AAPL", "TSLA"],
...
if st.session_state.get('connected'):
    ...
    if st.session_state.get('balance'):
        ...
        st.info("Click 'Refresh Real Balance' to load your account balance")
else:
    st.warning("Please connect to IBKR first")

# Trading (simple for now)
if auto_trading and st.session_state.get('connected') and (continuous or burst):
    st.success("🚀 Auto Trading Active - Check TWS for orders")

st.subheader("📜 Status")
with st.container():
    st.markdown('<div class="box">', unsafe_allow_html=True)
    st.info("Check **TWS → Trades tab** to see real activity")
