...
with col2:
    st.markdown('<div class="box"><h3>💰 Balance</h3><h2 id="balance-display">$ --.--</h2></div>', unsafe_allow_html=True)
...
if st.session_state.connected:
    st.success("✅ Connected - Ready to Trade")
else:
...
if auto_trading and st.session_state.connected and (continuous or burst):
    ...
    st.success(f"🚀 Sending REAL orders to IBKR ({'Burst ⚡' if burst else 'Continuous'})")
        ...
        with placeholder.container():
            for sym in markets:
                if random.random() < 0.45:
                    # === Proper Contract Creation ===
                    if sym == "XAUUSD":
                        contract = Forex("XAUUSD")
                    elif "." in sym:                    # Forex pairs
                        contract = Forex(sym)
                    ...
                    try:
                        ...
                        st.markdown(f"<h4 style='color:#00ff88;'>✅ REAL ORDER PLACED: {signal} {size} {sym}</h4>", unsafe_allow_html=True)
                    except Exception as e:
                        st.error(f"Order failed for {sym}: {e}")
        
        time.sleep(0.6 if burst else 1.8)

st.subheader("📜 Trade Log")
with st.container():
    st.markdown('<div class="box">', unsafe_allow_html=True)
    st.info("Check **TWS → Trades tab** and **Orders tab** to see real activity")
