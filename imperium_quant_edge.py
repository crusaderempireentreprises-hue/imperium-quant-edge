...
st.markdown("**Smart Adaptive AI - Clear Learning**")
...
markets = st.sidebar.multiselect("Base Markets (AI will learn & expand)", 
...
if auto_trading and st.session_state.get('connected') and (continuous or burst):
    st.success("🧠 Smart Adaptive AI is Learning & Adapting")
    ...
    while continuous or burst:
        with placeholder.container():
            for sym in markets:
                ...
                adapt_prob = 0.35 + (perf / 40)   # Favors good markets
                ...
                if random.random() < adapt_prob:
                    try:
                        ...
                        signal = "BUY" if perf > 0 else "SELL"
                        ...
                        pass
        time.sleep(0.6 if burst else 1.4)

# Learning Scores
st.subheader("🧠 AI Learning Scores")
if st.session_state.get('performance'):
    perf_df = pd.DataFrame(list(st.session_state.performance.items()), columns=["Market", "Learning Score"])
    st.dataframe(perf_df.sort_values("Learning Score", ascending=False), use_container_width=True)

st.subheader("📜 Trade Log")
with st.container():
    st.markdown('<div class="box">', unsafe_allow_html=True)
    st.info("The AI is learning which markets are profitable")
