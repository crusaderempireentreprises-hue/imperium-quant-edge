...
def execute_cycle():
    ...
    for sym in ["EUR.USD", "GBP.USD", "USDCAD"]:
        try:
            contract = Forex(sym.replace(".", ""))
            ib.qualifyContracts(contract)
            
            size = 25000                    # Exact size that worked for you
...
if col1.button("🚀 START CONTINUOUS TRADING", type="primary", use_container_width=True):
    add_log("Continuous mode STARTED - watch TWS")
    while True:
        execute_cycle()
        time.sleep(10)
...
st.caption("Check TWS → Orders tab after clicking buttons")