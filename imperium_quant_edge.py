...
st.markdown("**Smart Adaptive AI - Trading Fixed**")
...
risk_pct = st.sidebar.slider("Risk % per Trade", 0.25, 5.0, 0.8) / 100
...
if st.session_state.get('trading_active', False):
    ...
    with placeholder.container():
        ...
        st.metric("Current Cycle", "Running")

st.subheader("📜 Trade Log")
with st.container():
    st.markdown('<div class="box">', unsafe_allow_html=True)
    st.info("Click 'Start Trading Now' after enabling Auto Trading")
    st.markdown('</div>', unsafe_allow_html=True)

st.button("🛑 KILL SWITCH — Stop Trading", type="primary")