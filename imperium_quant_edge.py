...
client_id = st.sidebar.number_input("Client ID (Change if Error 326)", value=100)  # Higher default
...
if auto_trading and st.session_state.get('connected') and continuous:
    st.success("🚀 Continuous Smart Adaptive AI Running (Safe Mode)")
    placeholder = st.empty()
    
    while continuous:
        with placeholder.container():
            try:
                st.session_state.ib.reqGlobalCancel()  # Clean before new cycle
            except:
                pass
...
with st.container():
    ...
    st.info("""
    • If Error 326 appears → Increase Client ID (try 101, 102, etc.)
    • Always click "Cancel All Open Orders" before starting Continuous mode
    • AI learns and adapts automatically
    """)
