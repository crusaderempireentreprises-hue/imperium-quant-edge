...
markets = st.sidebar.multiselect("Markets", 
    ["XAUUSD", "EUR.USD", "USD.CAD", "BTCUSD", "AAPL"],
...
if auto_trading and st.session_state.connected and (continuous or burst):
    ...
    st.success("🚀 Sending REAL orders to IBKR Paper Account...")
            ...
            for sym in markets:
                if random.random() < 0.45:
                    ...
                    try:
                        ...
                        st.markdown(f"<h4 style='color:#00ff88;'>✅ REAL ORDER SENT: {signal} {size} of {sym}</h4>", unsafe_allow_html=True)
                    except Exception as e:
...
with st.container():
    ...
    st.info("Check **TWS → Trades tab** to see real executions")
