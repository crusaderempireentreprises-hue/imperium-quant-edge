...
st.markdown("**Real IBKR Trading - EUR Fixed**")
...
markets = st.sidebar.multiselect("Select Markets", 
    ["XAUUSD", "EUR.USD", "USD.CAD", "GBP.USD", "USD.JPY", "AUD.USD", "AAPL", "TSLA"],
                ...
                if random.random() < 0.45:
                    try:
                        ...
                        if sym == "XAUUSD":
                            ...
                            # Remove dot for ib_async (important fix for EUR.USD etc.)
                            clean_pair = sym.replace(".", "")
                            contract = Forex(clean_pair)
                        else:
