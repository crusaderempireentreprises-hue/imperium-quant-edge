...
def generate_candles():
    ...
    base = 100 + np.cumsum(np.random.randn(60) * 0.8)
    df = pd.DataFrame({
        'datetime': dates,
        'open': base,
        'high': base + np.abs(np.random.randn(60) * 2),
        'low': base - np.abs(np.random.randn(60) * 2),
        'close': base + np.random.randn(60) * 1.2
    })
    return df
...
def play_sound(signal):
    if signal == "BUY":
        st.markdown('<audio autoplay="true"><source src="https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3" type="audio/mpeg"></audio>', unsafe_allow_html=True)
    else:
        st.markdown('<audio autoplay="true"><source src="https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3" type="audio/mpeg"></audio>', unsafe_allow_html=True)
...
if auto_trading:
    st.success("🤖 Auto Trading is ACTIVE (with sound alerts)")
    ...
    for _ in range(6):
        with st.spinner(f"AI analyzing {symbol}..."):
            ...
            if signal:
                ...
                pnl = random.uniform(-0.045, 0.085)
                play_sound(signal)
...
if st.session_state.trade_log:
    ...
    st.info("Turn on Auto Trading to begin.")

st.button("🛑 KILL SWITCH — Stop All Trading", type="primary")

st.caption("Imperium Quant Edge • Published Version • Personal Use Only")