            ...
            try:
                ...
                if len(bars) >= 60:
                    ...
                    if signal:
                        ...
                        sl = last.close * (1 - stop_loss_pct) if signal == "BUY" else last.close * (1 + stop_loss_pct)
                        tp = last.close * (1 + take_profit_pct) if signal == "BUY" else last.close * (1 - take_profit_pct)

                        bracket = ib.bracketOrder(parent, 
                                                StopOrder("SELL" if signal == "BUY" else "BUY", size, sl),
                                                LimitOrder("SELL" if signal == "BUY" else "BUY", size, tp))

                        for o in bracket:
                            ib.placeOrder(contract, o)

                        st.success(f"✅ AI AUTO {signal} {size} of {symbol}")
                        st.session_state.trade_log.append({"time": datetime.now().strftime("%H:%M"), "symbol": symbol, "action": signal})
            except Exception as e:
                st.error(f"Error: {e}")
...
with tab2:
    st.header("Portfolio")
    if "Paper" in mode:
        st.metric("Simulated Balance", "$1,000 Fake CAD")
    if ib.isConnected():
...
with tab4:
    st.markdown("**Supported Markets**: Gold, Silver, Oil, Forex, US/Canadian Stocks, Bitcoin, Ethereum")

if ib.isConnected():
    ib.sleep(0.5)

st.caption("Imperium Quant Edge • Personal Use Only")