...
with col2:
    st.markdown('<div class="box"><h3>💰 IBKR Real Balance</h3><h2 id="balance-display">$ --.--</h2></div>', unsafe_allow_html=True)
...
markets = st.sidebar.multiselect("Select Markets", 
    ["XAUUSD", "EUR.USD", "USD.CAD", "BTCUSD", "AAPL", "TSLA"],
    ...
    try:
        ...
        for item in summary:
            if item.tag == "NetLiquidation":
                ...
                st.markdown(f'<div class="box"><h3>💰 Real IBKR Balance: ${balance:,.2f}</h3></div>', unsafe_allow_html=True)
    except:
        st.info("Balance will update after trading starts")
else:
            ...
            for sym in markets:
                if random.random() < 0.45:
                    try:
                        ...
                        size = max(1, int(1000 * risk_pct / 100))   # You can change this later
                        
                        order = MarketOrder(signal, size)
                        trade = ib.placeOrder(contract, order)
                        
                        st.markdown(f"<h4 style='color:#00ff88;'>✅ REAL ORDER: {signal} {size} {sym}</h4>", unsafe_allow_html=True)
                    except Exception as e:
                        st.error(f"Order error on {sym}: {e}")
...
with st.container():
    ...
    st.info("Check **TWS → Trades tab** and **Orders tab** to see real executions")
