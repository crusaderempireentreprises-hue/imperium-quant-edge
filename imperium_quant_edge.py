...
st.markdown("**Clean Smart Adaptive AI**")
...
if auto_trading and st.session_state.get('connected') and (continuous or burst):
    st.success("🧠 Smart Adaptive AI Running")
            ...
            for sym in markets:
                ...
                if random.random() < adapt_prob:
                    try:
                        ...
                        st.markdown(
                            f"<h4 style='color:#00ff88;'>"
                            f"✅ SMART TRADE: {signal} {size} {sym} (Score: {perf:.1f})"
                            f"</h4>", 
                            unsafe_allow_html=True
                        )
...
with st.container():
    ...
    st.info("The AI is learning which markets perform best")
