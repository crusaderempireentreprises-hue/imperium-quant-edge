...
def place_order(symbol, side):
    ...
    try:
        add_log(f"→ Sending {side} {abs(units)} {symbol}...")
        r = OrderCreate(ACCOUNT_ID, data=data)
        response = st.session_state.client.request(r)
        
        add_log(f"✅ RAW RESPONSE: {response}")
        st.success(f"Order Response for {symbol}: {response}")
        
    except Exception as e:
        add_log(f"❌ API ERROR {symbol}: {str(e)}")
