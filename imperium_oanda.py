...
ACCESS_TOKEN = "YOUR_NEW_REGENERATED_API_TOKEN"
...
st.set_page_config(page_title="Imperium OANDA Hybrid AI v1.3.1", layout="wide")
st.title("⚔️ IMPERIUM OANDA HYBRID AI v1.3.1")
...
def compute_score_components(m: MarketState, spread: float):
    ...
    regime_score = 0.2 if m.regime == "UP_TREND" else -0.2 if m.regime == "DOWN_TREND" else 0.0
    ...
    momentum_score = mom * 50.0

    spread_penalty = -0.3 if spread > m.last * 0.0004 else 0.0

    return momentum_score, regime_score, vol_score + spread_penalty, memory_score
...
def run_cycle():
    ...
    for m in markets.values():
        ...
        quality = 0
        if abs(m.mom_ema) > 0.0005: quality += 1
        if 0.0002 < m.vol_ema < 0.002: quality += 1
        if spread_est < m.last * 0.0003: quality += 1
        if m.atr < m.last * 0.002: quality += 1
