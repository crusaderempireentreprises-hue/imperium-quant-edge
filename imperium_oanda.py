...
st.set_page_config(page_title="Imperium OANDA Hybrid AI v1.4", layout="wide")
st.title("⚔️ IMPERIUM OANDA HYBRID AI v1.4 — Dynamic Universe")
...
# active markets dict (built from base + dynamic)
...
# ============================================================
# MARKET DISCOVERY ENGINE (DYNAMIC UNIVERSE)
# ============================================================

def discover_markets():
    """
    Scan OANDA instruments, filter for FX + metals, measure spread quality,
    and select top candidates to add as dynamic symbols.
    """
    ...
    # Filter to FX + metals, exclude our base symbols (we already trade them)
    candidate_symbols = []
    ...
    # Get pricing snapshot for candidates (batch in chunks to avoid URL length issues)
    spreads_info = []
    chunk_size = 25
    ...
    # Filter for tight spreads and reasonable price levels
    filtered = [
        (sym, spread_ratio)
        for sym, spread_ratio, mid in spreads_info
        if spread_ratio < 0.0005  # ~0.05% spread or tighter
    ]
    ...
    # Ensure market states exist for new symbols
...
def compute_units(m: MarketState, nav: float) -> int:
    ...
    # dynamic symbols trade slightly smaller size
